...
function extractTextOperators(raw, fontMaps = {}) {
  ...
  const textTokenPattern = /\/([A-Za-z]\w*)\s+[\d.]+\s+Tf|<([0-9A-Fa-f\s]+)>\s*Tj|\[((?:\s*(?:<[\dA-Fa-f\s]+>|-?\d+(?:\.\d+)?)\s*)+)\]\s*TJ/g;
...
async function extractTextFromDocxBuffer(buffer) {
  try {
    ...
    console.warn("DOCX mammoth extraction failed, using XML fallback:", error.message);
  }
...
function normalizeText(value) {
  const cleaned = String(value || "")
    ...
    .replace(/\s+/g, " ")
    .trim();
  return addResumeStructure(repairJoinedWords(repairSpacedNumbers(repairSpacedLetters(cleaned)))
    ...
    .replace(/\bSept ember\b/g, "September")
...
function repairSpacedNumbers(text) {
  return String(text || "").replace(/\b\d+(?:\s+\d+)+\b/g, (match) => match.replace(/\s+/g, ""));
}

function repairJoinedWords(text) {
  return String(text || "")
    .replace(/([a-z])([A-Z])/g, "$1 $2")
    .replace(/\b(Work)(Experience)\b/g, "$1 $2")
    .replace(/\b(Professional)(Experience)\b/g, "$1 $2")
    .replace(/\b(Technical)(Skills)\b/g, "$1 $2")
    .replace(/\b(Open)(Source)\b/g, "$1 $2")
    .replace(/\b(Full)(Stack)\b/g, "$1 $2")
    .replace(/\b(Software)(Engineer|Developer)\b/g, "$1 $2")
    .replace(/\b(Senior)(Software)\b/g, "$1 $2")
    .replace(/\b(Bank)(of)(America)\b/g, "$1 $2 $3")
    .replace(/\b(United)(Health)(Group)\b/g, "$1 $2 $3")
    .replace(/\b(South)(Arkansas)(College)\b/g, "$1 $2 $3")
    .replace(/\b(Texas)(Tech)(University)\b/g, "$1 $2 $3")
    .replace(/\b(Stanford)(University)\b/g, "$1 $2")
    .replace(/\b(Jawaharlal)(Nehru)(Technological)(University|Univeristy)?\b/g, "Jawaharlal Nehru Technological University")
    .replace(/\b(Programming)(and)(Scripting)\b/g, "$1 $2 $3")
    .replace(/\b(Web)(Technologies)\b/g, "$1 $2")
    .replace(/\b(Cloud)(Technologies)\b/g, "$1 $2")
    .replace(/\b(Testing)(Tools)\b/g, "$1 $2")
    .replace(/\b(Distributed)(Technologies)\b/g, "$1 $2");
}
