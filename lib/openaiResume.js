  ...
  try {
    const result = await callOpenAI({
      ...
      input:
        ...
        `Source employer names that must remain represented: ${sourceResume.experience.map((item) => item.company).filter(Boolean).join(", ") || "none detected"}.\n\n` +
...
function normalizeTailored(value, { targetPageCount = 1, aiUsed = false } = {}) {
  ...
  const sectionedExperience = compact
    ? experience.map((item) => ({ ...item, bullets: item.bullets.slice(0, 2) }))
...
function mergeGeneratedResume(source, generated) {
  ...
  return {
    ...
    coreSkills,
    experienceBullets: experience.flatMap((item) => item.bullets).slice(0, 18)
  };
}

function mergeMissingExperience(generatedExperience, sourceExperience) {
  ...
  for (const sourceItem of sourceExperience) {
    ...
    merged.push({
      ...sourceItem,
      bullets: sourceItem.bullets.slice(0, 2)
    });
    seen.add(key);
  }
...
function validateTailoredResumeQuality(resume, { targetPageCount }) {
  ...
  // Relaxed requirements: minimum 1 bullet for 1-page, 2 for 5+ page, 2 otherwise
  const minExperienceBullets = targetPageCount <= 1 ? 1 : targetPageCount >= 5 ? 2 : 2;
  const combinedText = [
...
function monthRangePattern() {
  return /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Sept|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?\s+\d{4}\s*-\s*(?:Present|Current|Now|(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Sept|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\.?\s+\d{4})\b/i;
}

function actionBulletPattern() {
  return /\s+-\s+(?=(?:Debugged|Resolved|Contributed|Engaged|Documented|Developed|Applied|Added|Created|Performed|Designed|Built|Deployed|Used|Implemented|Led|Managed|Owned|Improved|Reduced|Increased|Automated|Analyzed|Partnered|Collaborated|Delivered|Shipped|Uncovered|Segmented|Integrated|Migrated|Configured|Maintained|Optimized|Supported|Tested|Enhanced|Architected)\b)/;
}
