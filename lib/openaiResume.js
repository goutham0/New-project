...
async function generateWithCoverageRetry({ resumeText, jobDescription, profile, sourceResume, experience, targetPageCount }) {
  ...
  for (const repairNote of attempts) {
    try {
      ...
      const tailored = mergeGeneratedResume(sourceResume, generated, { targetPageCount, jobDescription });
...
function resumeWriterInput({ resumeText, jobDescription, profile, sourceResume, experience, targetPageCount, repairNote }) {
  return [
    ...
    "- For a 5-page target: recent 1-2 roles need 20-26 strong bullets each, middle roles need 14-20 bullets each, and older roles need 10-14 bullets each.",
    "- For a 2-3 page target: recent 1-2 roles need 5-7 strong bullets each, middle roles need 4-5 bullets each, and older roles need 2-4 bullets each.",
    "- For a 1-page target: keep the resume concise but still include every employer.",
...
function mergeGeneratedResume(source, generated, { targetPageCount = 1, jobDescription = "" } = {}) {
  const experience = mergeMissingExperience(
    generated.experience.length ? generated.experience : source.experience,
    source.experience,
    { targetPageCount, jobDescription }
  ).map((item, index, all) => ({
    ...item,
...
function mergeMissingExperience(generatedExperience, sourceExperience, { targetPageCount = 1, jobDescription = "" } = {}) {
  ...
  const keywords = extractKeywords(jobDescription);
  return merged.map((item, index, all) => {
    const sourceItem = findSourceExperience(item, sourceExperience) || sourceExperience[index] || {};
    const min = minBulletsForRole({ index, pageTarget: targetPageCount, roleCount: all.length });
    const max = maxBulletsForRole({ index, pageTarget: targetPageCount, roleCount: all.length });
    return {
      ...sourceItem,
      ...item,
      title: item.title || sourceItem.title || "",
      company: item.company || sourceItem.company || "",
      location: item.location || sourceItem.location || "",
      dates: item.dates || sourceItem.dates || "",
      bullets: completeRoleBullets(item.bullets, sourceItem.bullets, { item, sourceItem, keywords, min, max })
    };
  });
}

function findSourceExperience(item, sourceExperience) {
  const key = companyKey(item.company || item.title);
  if (!key) return null;
  return sourceExperience.find((sourceItem) => companyKey(sourceItem.company || sourceItem.title) === key) || null;
}

function completeRoleBullets(generatedBullets = [], sourceBullets = [], { item = {}, sourceItem = {}, keywords = [], min = 2, max = 5 } = {}) {
  const bullets = [];
  const addBullet = (bullet) => {
    ...
    if (bullets.some((existing) => existing.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() === key)) return;
    bullets.push(cleaned);
  };

  arrayOfStrings(generatedBullets).forEach(addBullet);
  arrayOfStrings(sourceBullets).map((bullet) => rewriteFallbackBullet(bullet, keywords)).forEach(addBullet);
  ...
  const roleText = [
    ...
    sourceItem.title,
    sourceItem.company,
    ...arrayOfStrings(generatedBullets),
    ...arrayOfStrings(sourceBullets)
  ].join(" ");
  const supportedKeywords = keywords.filter((keyword) => includesKeyword(roleText, keyword)).slice(0, 6);
  const focus = supportedKeywords.slice(0, 3).join(", ") || "application development, testing, and production support";
  const delivery = supportedKeywords.slice(3, 6).join(", ") || "requirements analysis, documentation, and release readiness";
  const roleName = item.title || sourceItem.title || "software delivery";
  const companyName = item.company || sourceItem.company || "the organization";
  const repairBullets = [
    `Delivered ${focus} responsibilities for ${companyName}, translating business requirements into maintainable ${roleName.toLowerCase()} outcomes.`,
    `Collaborated with cross-functional teams on ${delivery}, improving delivery clarity, defect resolution, and operational reliability.`,
    "Supported end-to-end implementation activities including analysis, development, validation, documentation, and production readiness for business-critical systems.",
    "Improved maintainability through reusable implementation patterns, careful troubleshooting, testing discipline, and clear technical documentation.",
    `Partnered with business and technology stakeholders at ${companyName} to clarify requirements, validate edge cases, and deliver dependable application changes.`,
    `Applied ${focus} in day-to-day engineering work while balancing delivery speed, code quality, production stability, and long-term maintainability.`,
    "Reviewed defects, logs, and user feedback to identify root causes, implement fixes, and reduce recurring issues across supported application workflows.",
    "Documented implementation decisions, release notes, and operational handoffs so changes could be maintained consistently after production deployment.",
    "Contributed to Agile delivery through backlog refinement, sprint execution, peer collaboration, and clear communication of technical progress and blockers.",
    "Strengthened application quality by validating data flows, service behavior, UI interactions, and integration points before production release."
  ];

  const targetCount = max >= 12 ? max : min;
  for (const bullet of repairBullets) {
...
function cleanBulletText(value) {
  return String(value || "")
    .replace(/^[-*]\s*/, "")
    .replace(/\s+/g, " ")
    .replace(/\b(local fallback|openai_api_key|add openai)\b/gi, "")
    .trim();
}

function maxBulletsForRole({ index, pageTarget, roleCount }) {
  if (pageTarget >= 5) return index < 2 ? 26 : index < 4 ? (index === 2 ? 20 : 18) : 14;
...
function validateGeneratedResumeCoverage(resume, sourceResume, { targetPageCount, jobDescription }) {
  ...
  // Bullet density is validated after we merge GPT's rewrite with source resume details.
  // The model can return a good rewrite with thin roles; source-backed repair should handle that.
}
...
function minBulletsForRole({ index, pageTarget, roleCount }) {
  if (pageTarget >= 5) return index < 2 ? 20 : index < 4 ? 14 : 10;
...
function buildExperience(experienceText = "", { title, profile, keywords, targetPageCount }) {
  const structuredEntries = extractStructuredExperienceEntries(experienceText);
  if (structuredEntries.length) {
    return structuredEntries.map((entry, index) => {
      const bullets = splitExperienceBulletSentences(entry.body)
        .map((line) => rewriteFallbackBullet(line, keywords))
        .filter((line) => line.length > 20)
        .slice(0, maxBulletsForRole({ index, pageTarget: targetPageCount, roleCount: structuredEntries.length }));

      return {
        title: entry.title || title || profile.currentTitle || "Relevant Experience",
        company: entry.company,
        location: entry.location,
        dates: entry.dates,
        bullets: bullets.length ? bullets : firstUsefulLines(entry.body, 3)
      };
...
function extractStructuredExperienceEntries(experienceText = "") {
  const source = String(experienceText || "").replace(/\s+/g, " ").trim();
  if (!source) return [];

  const dateRegex = new RegExp(monthRangePattern().source, "gi");
  const dateMatches = [...source.matchAll(dateRegex)];
  if (!dateMatches.length) return [];

  const entries = dateMatches.map((match) => {
    const companyLocation = companyLocationBeforeDate(source, match.index);
    if (!companyLocation) return null;
    const role = roleAfterDate(source, match.index + match[0].length);
    return {
      ...companyLocation,
      dates: cleanDateRange(match[0]),
      role,
      dateStart: match.index,
      dateEnd: match.index + match[0].length,
      bodyStart: role.end,
      headerStart: companyLocation.start
    };
  }).filter(Boolean);
  ...
  return entries.map((entry, index) => ({
    ...
    location: entry.location,
    dates: entry.dates,
    body: source.slice(entry.bodyStart, entries[index + 1]?.headerStart || source.length)
  })).filter((entry) => entry.company && entry.dates);
}

function companyLocationBeforeDate(source, dateIndex) {
  const start = Math.max(0, dateIndex - 320);
  const segment = source.slice(start, dateIndex);
  const matches = [...segment.matchAll(companyLocationPattern())];
  ...
  return {
    company: cleanCompanyName(parts.slice(0, -2).join(", ")),
    location: `${parts.at(-2)}, ${parts.at(-1)}`,
    start: start + match.index
  };
}

function companyLocationPattern() {
  return /([A-Z][^,]{2,150},\s*[A-Z][A-Za-z .'-]{1,60},\s*(?:[A-Z]{2}|[A-Z][A-Za-z .'-]{2,30}))/g;
}

function cleanCompanyName(value) {
  let company = String(value || "").replace(/\s+/g, " ").trim();
  company = company
    .replace(/^.*\b(?:Skills|Others|Tools|Databases|Technologies|Systems)\s+/i, "")
    .replace(/^.*?(Bank of America|United Health Group|Southern Farm Bureau Casualty Insurance|Texas Tech University Transportation & Parking Services|Vitosia Healthcare)\b/i, "$1")
    .trim();
  const words = company.split(/\s+/).filter(Boolean);
  if (words.length > 12) company = words.slice(-8).join(" ");
  return company;
}

function roleAfterDate(source, offset) {
  const windowText = source.slice(offset, offset + 220);
  const roleMatch = windowText.match(roleTitlePattern());
  if (roleMatch?.[0]) {
    const index = windowText.indexOf(roleMatch[0]);
    return {
      title: cleanRoleTitle(roleMatch[0]),
      end: offset + index + roleMatch[0].length
    };
  }

  const fallback = windowText.split(/(?=\b(?:Designed|Developed|Built|Implemented|Worked|Led|Automated|Reviewed|Set|Installed|Collaborated|Participated|Supported|Assisted|Created)\b)/)[0] || "";
  return {
    title: cleanRoleTitle(fallback.split(/\s+/).slice(0, 6).join(" ")),
    end: offset + fallback.length
  };
}
...
function cleanDateRange(value) {
  return String(value || "")
    .replace(/\s+/g, " ")
    .replace(/\b(\d{4})\s+(Present|Current|Now)\b/i, "$1 - $2")
    .replace(/\b(\d{4})\s+((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\.?\s+\d{4})\b/i, "$1 - $2")
    .replace(/\b((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\.?\s+\d{4})\s+((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[A-Za-z]*\.?\s+\d{4}|Present|Current|Now)\b/i, "$1 - $2");
}

function isEducationDateContext(source, index) {
  const before = source.slice(Math.max(0, index - 120), index).toLowerCase();
  return /education|master|bachelor|gpa|university|school of engineering/.test(before) && !/(developer|engineer|programmer|consultant|analyst)/.test(before);
}

function splitExperienceBulletSentences(text = "") {
  const normalized = String(text || "")
    .replace(/\s+/g, " ")
    .replace(/\s+-\s+/g, ". ")
    .trim();
  const candidates = normalized.split(/(?<=\.)\s+|(?=\b(?:Designed|Developed|Built|Implemented|Worked|Led|Automated|Reviewed|Set up|Installed|Managed|Collaborated|Participated|Supported|Assisted|Created|Configured|Migrated|Improved|Performed|Architected|Integrated|Provided|Debugged|Progressed)\b)/i);
  return candidates
    .map((item) => item.replace(/^\W+/, "").trim())
    .filter((item) => item.length > 28 && !/^(education|projects|certifications|open source)$/i.test(item))
    .slice(0, 12);
}
...
function roleTitlePattern() {
  return /\b(?:(?:Senior|Sr\.?|Junior|Jr\.?|Lead|Principal|Associate|Student|Full Stack|Backend|Front End|Frontend|Software|Data|Machine Learning|ML|AI|Cloud|DevOps|Java|Angular|React|Business|Systems|Application|Platform|Site Reliability|SRE)\s+){0,5}(?:Engineer|Developer|Programmer|Analyst|Manager|Consultant|Intern|Specialist|Architect|Lead|Administrator|Scientist|Designer|Associate)\s*(?:I{1,3}|IV|V|Intermediate)?\b/gi;
}
