...
export async function generateTailoredResume({ resumeText, jobDescription, profile = {} }) {
  ...
  try {
    ...
    return mergeGeneratedResume(sourceResume, { ...normalizeTailored(result, { targetPageCount }), aiUsed: true });
  } catch (error) {
    return {
      ...sourceResume,
...
function normalizeTailored(value, { targetPageCount = 1, aiUsed = false } = {}) {
  const pageTarget = Number(value.targetPageCount || targetPageCount);
...
function buildExperience(experienceText = "", { title, profile, keywords, targetPageCount }) {
  ...
  return [{
    ...
    bullets: bullets.length
      ? bullets
      : [`Applied ${keywords.slice(0, 3).join(", ") || "role-relevant"} experience while preserving candidate-provided facts.`]
  }];
}
