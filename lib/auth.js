...
const cookieName = "applyfriend_session";
...
export function createAssistedApplyToken(user, application) {
  return sign({
    userId: user.id,
    applicationId: application.id,
    scope: "assisted_apply",
    expiresAt: Date.now() + 1000 * 60 * 60 * 6
  });
}

export async function userFromAssistedApplyToken(token) {
  const payload = verify(token);
  if (payload?.scope !== "assisted_apply" || !payload.userId || !payload.applicationId) return null;
  const user = await getUserById(payload.userId);
  return user ? { user, applicationId: payload.applicationId } : null;
}
